...
type LegacySavedBook = SavedBook & {
  format?: string;
};
...
const toIsoDate = (date: Date) => date.toISOString();
  ...
  const [book, setBook] = useState<LegacySavedBook | null>(null);
...
const flowerPreviewAssets: Record<string, any> = {
  ...
  useEffect(() => {
    const loadBook = async () => {
      ...
      const foundBook = books.find((b) => b.id === id) as
        | LegacySavedBook
        | undefined;

      if (foundBook) {
        const startingFormats = foundBook.formats?.length
          ? foundBook.formats
          : foundBook.format
            ? [foundBook.format]
            : [];
        ...
        const legacyNotes =
          foundBook.notes && foundBook.notes.length > 0
            ? foundBook.notes
            : foundBook.quickNotes
              ? [foundBook.quickNotes]
              : [];

        setBook(foundBook);
        setSelectedFormats(startingFormats);
        setStatus(foundBook.status || "");
        setStartedAt(foundBook.startedAt || "");
  ...
  setNoteDeleteIndex(index);
};
const confirmDeleteNote = () => {
  if (noteDeleteIndex === null) return;

  const updatedNotes = notes.filter((_, i) => i !== noteDeleteIndex);
  setNotes(updatedNotes);

  if (editingNoteIndex === noteDeleteIndex) {
    setEditingNoteIndex(null);
    setNoteDraft("");
  }
...
const renderSaveSuccessModal = () => (
  ...
  if (returnTo === "library") {
    ...
    router.replace("/(tabs)");
        ...
        >
          <Text style={styles.modalDoneText}>Lovely</Text>
        </TouchableOpacity>
      </View>
    </View>
  </Modal>
...
const renderDeleteBookModal = () => (
  ...
  >
    <View style={styles.modalOverlay}>
      <View style={styles.alertCard}>
        <Text style={styles.alertTitle}>Remove Book?</Text>
        ...
        <View style={styles.alertButtonRow}>
          ...
          <TouchableOpacity
            style={styles.alertDangerButton}
            onPress={confirmDeleteBook}
          >
            <Text style={styles.alertDangerText}>Remove</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
...
const renderDeleteNoteModal = () => (
  <Modal
    ...
    transparent
    animationType="fade"
    onRequestClose={() => setNoteDeleteIndex(null)}
  >
    <View style={styles.modalOverlay}>
      <View style={styles.alertCard}>
        <Text style={styles.alertTitle}>Delete Note?</Text>

        <Text style={styles.alertMessage}>
          This note will disappear into the void 🌿
        </Text>
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        <ActivityIndicator size="large" color="#234028" />
      </View>
    );
  }
